...
\begin{align*}
  x &\in B \\
  \left(y^1, \dots, y^s\right) = &\in F \\
  \left(y^1_x, \dots, y^s_x\right) &\in F_1.
\end{align*}
...
\begin{thm} \label{thm:linearized-general-symmetry}
  ...
  \begin{equation*} \label{eq:linearized-general-symmetry}
    \eval{\prolong{X}{1}(\vect{y}_x - \vect{\omega}(z))}_{\vect{y}_x = \vect{\omega}(z)} = 0,
  \end{equation*}
...
For a system of ODE:s, the characteristic becomes a function \(\vect{Q}(\prolong{z}{1}) = \left(Q^1(\prolong{z}{1}), \dots, Q^s(\prolong{z}{1})\right)\) defined by
\begin{equation*}
...
\begin{multline*}
  \eval{\prolong{(c_1 X_1 + c_2 X_2)}{1}(\vect{\Delta}(\prolong{z}{1}))}_{\vect{\Delta}(\prolong{z}{1}) = 0} =\\
  = c_1 \eval{\prolong{X_1}{1}(\vect{\Delta}(\prolong{z}{1}))}_{\vect{\Delta}(\prolong{z}{1}) = 0} + c_2 \eval{\prolong{X_2}{1}(\vect{\Delta}(\prolong{z}{1}))}_{\vect{\Delta}(\prolong{z}{1}) = 0} =
  0
\end{multline*}
at any point \(z \in E\).
...
\begin{equation*} %\label{eq:lie-bracket}
  \comm{X_1}{X_2} = \left(X_1(\xi_2) - X_2(\xi_1)\right) \partial_x + \left(X_1(\vect{\eta}_1) - X_2(\vect{\eta}_1)\right) \cdot \partial_{\vect{y}},
\end{equation*}
where \(X_1 = \xi_1 \partial_x + \vect{\eta}_2 \cdot \partial_{\vect{y}}\) and \(X_2 = \xi_2 \partial_x + \vect{\eta}_2 \cdot \partial_{\vect{y}}\).
...
If \(X\) is a trivial symmetry, \(X_{\bar{\vect{Q}}} = 0\).
...
Similarly, all symmetries on the form \(\vect{\eta} \cdot \partial_{\vect{y}}\) form a subalgebra \(\mathfrak{g}^0\), since
\begin{equation*}
...
\begin{multline*}
  ...
  = \comm{\xi X_{\text{T}}}{\tilde{\xi} X_{\text{T}}} + \bar{Q} \tilde{\xi}_y X_{\text{T}} + \tilde{\xi} \comm{X_{\bar{\vect{Q}}}}{X_{\text{T}}} =
...
The concept of invariants can be generalized.
...
A function \(\vect{I}\) is called an invariant of a first order ODE if it is invariant of its corresponding vector field.
...
\begin{defn}[Universal invariant]
  A function
  \begin{equation*}
    \vect{J}: E \to \reals^n
  \end{equation*}
  is called a universal invariant of a vector field corresponding to the infinitesimal generator \(X\) if any invariant \(\vect{I}\) of that vector field can be written as
  \begin{equation*}
    \vect{I}(x, \vect{y}) = F(\vect{J}(x, \vect{y})),
  \end{equation*}
...
\end{thm}
Here \(\partial_{\vect{y}} \vect{J}\) will for all intents and purposes be the Jacobian, excluding the derivative in \(x\).
...
\begin{cor} \label{cor:reduced-characteristic-decomposition}
  ...
  \end{equation*}
  where \(\vect{J}: E \to \reals^s\) is a universal invariant and \(\vect{I}: E \to \reals^s\) an arbitrary invariant of the vector field corresponding to the infinitesimal generator \(X_{\text{T}}\).
\end{cor}
