...
It is mainly based on material from two sources.
The first source is \cite{hydon2000symmetry}, which serves as a great introductory text on the subject of symmetry methods.
The second one is \cite{ovsiannikov1982group}, which treats the subject stringently and serves as a good reference point for proper treatment of the subject.
The notation used in the thesis will be based on the notation in \cite{hydon2000symmetry}, employing partial notational concepts from \cite{ovsiannikov1982group} when further clarity is desired.
...
%-----------------------------------------------------------------------------
\subsection{A note on differentiation and vector spaces}
...
Since symmetry methods deal with transformations, \(y\) will be formally be treated as a point in \(\reals\).
To make the point treatment of the dependent variable \(y\) consistent with the function treatment often used when dealing with differential equations, some concepts must be introduced.

The \(xy\)-plane previously mentioned can be seen as consisting of two components: the independent space \(X = \reals\) and the dependent space \(Y = \reals\).
The plane then is the product space \(Z = X \cross Y = \reals^2\).
To be able to formulate differential statements the space \(Y_1 = \reals\), called the first prolongation (or continuation) of \(Y\) with respect to \(X\), is introduced.
...
A first order ODE can then be written as an equation in the space \(Z_1 = Z \cross Y_1 = \reals^3\), also known as the prolongation of \(Z\) to the first order.
The ODE \ref{eq:first-order-ode} is equivalent to
\begin{equation}
  \varepsilon(\underset{1}{z}) = y' - \omega(x,y) = 0,
\end{equation}
where \(\underset{1}{z} = (x,y,y') \in Z_1\).
When dealing with an ODE of higher order the space \(Z\) can be prolonged to an appropriate order, following the formula
\begin{equation}
  Z_k = X \cross Y \cross Y_1 \cross \dots \cross Y_k,
\end{equation}
with elements
\begin{equation}
...
To mirror the behavior of differentiation of the function \(y(x)\), a differential operator on \(Z_k\) is introduced, called the total derivative (or the operator of exact differentiation)
\begin{equation}
  D_x = \partial_x + y' \cdot \partial_y + y'' \cdot \partial_{y'} + \dots + y^{(l)} \cdot \partial_{y^{(l-1)}} + \dots .
\end{equation}
\(D_x\) will act on expressions in \(Z_k\) in the same way that \(\dv{x}\) would act on equivalent expressions consisting of \(x\), \(y(x)\) and derivatives of \(y(x)\) in \(x\).
