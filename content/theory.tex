...
\begin{equation}
  \prolong{\gamma_u}{k} = \qty{\prolong{z}{k} = \qty(\fixed{x}, y, y', \dots, y^{(k)})\qq*{:} y^{(l)} = \dv[l]{u}{x}{(\fixed{x})}\qc \fixed{x} \in B\qc l = 0, \dots, k}
\end{equation}
...
\begin{equation}
  \eval{\dv{\epsilon} \Gamma \circ \Gamma_\epsilon}_{\epsilon = 0} = X \Gamma.
\end{equation}
...
\begin{defn}[Point symmetry] \label{defn:general-symmetry}
  ...
  \begin{equation}
    \dv{\vb{y}}{x} = \vb*{\omega}(z)
    \implies
    \dv{\hat{\vb{y}}}{\hat{x}} = \vb*{\omega}(\hat{z}).
  \end{equation}
\end{defn}
