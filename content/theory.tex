...
All differentiable functions \(u: X \to Y\) have a single equivalent function \(\prolong{u}{1}: X \to \prolong{Y}{1}\) called the first prolongation of \(u(x)\).
The prolongation is defined as
% FIXME: Avoid relax
\begin{equation}
  \prolong{u}{1}(x) = \qty(u(x), \dv{u}{x}\relax(x)).
\end{equation}
...
\begin{equation}
  \prolong{\gamma}{1} = \qty{\qty(x, \prolong{u}{1}(x)): x \in X},
\end{equation}
where \(u\) is the function corresponding\dots
...
\begin{equation} \label{eq:first-order-jet-ode}
  \varepsilon(\prolong{z}{1}) = y' - \omega(x,y) = 0,
\end{equation}
