...
\begin{align*}
  ...
  \end{split}
\end{align*}
where \(A^1(t), \dots, A^s(t)\) are the measurements of species at time \(t\), and \(\vect{\theta} = \left(\theta^1, \dots, \theta^m\right)\) are a collection of parameters.
...
The autonomous Gompertz ODE \labelcref{eq:rough-gompertz-autonomous-ode} is most easily solved using the variable substitution \(y(t) = \ln\left(W(t)/K\right)\) and then solving the resulting ODE.
